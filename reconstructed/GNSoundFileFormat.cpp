// Reconstructed implementation of GNSoundFileFormat
// From MikeCore binary — reverse-engineered pseudocode

#include "GNSoundFileFormat.h"

// ============================================================
// @00135840 — 571 bytes
// str: ""GNSoundFileFormat""
// str: ""_soundFileFormat""
// str: ""MDExportControllerHelper""
// ============================================================

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

undefined * FUN_00135840(void)

{
  int iVar1;
  
  if (DAT_026e1e00 == '\0') {
    iVar1 = ___cxa_guard_acquire();
    if (iVar1 != 0) {
      _DAT_026e1dd0 = _DAT_0238fcc0;
      uRam00000000026e1dd8 = _UNK_0238fcc8;
      _DAT_026e1de0 = _DAT_0238ff10;
      uRam00000000026e1de4 = _UNK_0238ff14;
      uRam00000000026e1de8 = _UNK_0238ff18;
      uRam00000000026e1dec = _UNK_0238ff1c;
      _DAT_026e1df0 = 0x900000008;
      _DAT_026e1df8 = 10;
      DAT_026e1dfc = 1;
      ___cxa_guard_release();
    }
  }
  if (DAT_026e1e38 == '\0') {
    iVar1 = ___cxa_guard_acquire();
    if (iVar1 != 0) {
      _DAT_026e1e08 = "GNSoundFileFormat";
      _DAT_026e1e10 = 0xb;
      DAT_026e1e14 = DAT_026e1dfc;
      _DAT_026e1e18 = &DAT_026e1dd0;
      _DAT_026e1e20 = &DAT_026e1d70;
      _DAT_026e1e28 = 0;
      uRam00000000026e1e30 = 0;
      ___cxa_guard_release();
    }
  }
  FUN_00e876a0();
  if (DAT_026e2ee0 == '\0') {
    iVar1 = ___cxa_guard_acquire();
    if (iVar1 != 0) {
      if (DAT_026e2e98 == '\0') {
        iVar1 = ___cxa_guard_acquire();
        if (iVar1 != 0) {
          _DAT_026e2de8 = FUN_00d4fe50();
          _DAT_026e2dd0 = "MDExportControllerHelper";
          _DAT_026e2dd8 = 0x30;
          _DAT_026e2de0 = FUN_00135230;
          _DAT_026e2df0 = 0;
          uRam00000000026e2df8 = 0;
          _DAT_026e2e00 = 0;
          uRam00000000026e2e08 = 0;
          _DAT_026e2e10 = 0;
          uRam00000000026e2e18 = 0;
          _DAT_026e2e20 = 0;
          uRam00000000026e2e28 = 0;
          _DAT_026e2e30 = 0;
          uRam00000000026e2e38 = 0;
          _DAT_026e2e40 = 0;
          uRam00000000026e2e48 = 0;
          _DAT_026e2e50 = 0;
          uRam00000000026e2e58 = 0;
          _DAT_026e2e60 = 0;
          uRam00000000026e2e68 = 0;
          _DAT_026e2e70 = 0;
          uRam00000000026e2e78 = 0;
          _DAT_026e2e80 = 0;
          uRam00000000026e2e88 = 0;
          _DAT_026e2e90 = 0;
          ___cxa_guard_release();
        }
      }
      _DAT_026e2ea0 = "_soundFileFormat";
      _DAT_026e2ea8 = &DAT_026e2dd0;
      _DAT_026e2eb0 = 0;
      _DAT_026e2eb8 = 0x6500;
      _DAT_026e2ec0 = "GNSoundFileFormat";
      _DAT_026e2ec8 = &DAT_026e1e08;
      _DAT_026e2ed0 = 0;
      uRam00000000026e2ed8 = 0;
      ___cxa_guard_release();
    }
  }
  return &DAT_026e2ea0;
}



