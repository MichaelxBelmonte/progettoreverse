// Function: FUN_01cd1d40
// Address: 01cd1d40
// Size: 2208 bytes
// Class: GNString
// String references:
//   "color format mismatch"
//   "color format mismatch %@"
//   "color range mismatch %@"
// === GNString properties ===
//   bool            _dragEnabled
//   bool            _dropEnabled
//   double          _transferStartTime
//   double          _transferEndTime
//   double          _userStartTime
//   double          _userEndTime


uint64_t FUN_01cd1d40(uint32_t param_1,int64_t *param_2)

{
  int64_t lVar1;
  int64_t *plVar2;
  bool bVar3;
  int64_t lVar4;
  char cVar5;
  int iVar6;
  void*puVar7;
  uint64_t uVar8;
  uint64_t this_ptr;
  void*puVar9;
  float extraout_XMM0_Da;
  uint32_t extraout_XMM0_Da_00;
  uint32_t uVar10;
  uint32_t extraout_XMM0_Da_01;
  uint32_t extraout_XMM0_Da_02;
  uint32_t extraout_XMM0_Da_03;
  uint32_t extraout_XMM0_Da_04;
  float extraout_XMM0_Da_05;
  float fVar11;
  float extraout_XMM0_Da_06;
  int64_t local_a8;
  char local_a0;
  void*local_98;
  char local_90;
  void*local_88;
  char local_80;
  void*local_78;
  char local_70;
  uint local_64;
  int local_60;
  int local_5c;
  void*local_58;
  uint local_50;
  int64_t local_48;
  char local_40;
  
  lVar4 = g_027eec90;
  local_64 = 0xff;
  local_60 = 0xff;
  local_5c = 0xff;
  if (*param_2 == 0) {
    if (g_027eec90 != 0) {
      param_1 = FUN_00d50b00();
    }
    FUN_00cc7b40(param_1,g_025908a0);
    if (lVar4 != 0) {
      FUN_00d50b20();
    }
  }
  iVar6 = FUN_00d8c7a0();
  lVar4 = g_0276cc80;
  if (iVar6 == 6) {
    fVar11 = extraout_XMM0_Da;
    if ((g_028b7520 == '\0') &&
       (iVar6 = ___cxa_guard_acquire(), fVar11 = extraout_XMM0_Da_06, iVar6 != 0)) {
      g_028b7518 = '\0';
      g_028b7519 = '\0';
      ram_00000000028b751a = 0;
      g_028b7510._0_7_ = 0;
      g_028b7510._7_1_ = 0;
      ___cxa_atexit(extraout_XMM0_Da_06,0);
      fVar11 = (float)___cxa_guard_release();
    }
    plVar2 = (int64_t *)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510);
    if ((plVar2 == (int64_t *)0x0) || (g_028b7519 == '\0')) {
      FUN_00e8cb50();
      if (CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510) == 0) {
        puVar7 = (void*)FUN_00e8fc40();
        FUN_00d4ff40();
        *puVar7 = &g_0257ef30;
        puVar7[5] = 0;
        puVar7[2] = 0;
        puVar7[3] = 0;
        *(void*)(puVar7 + 4) = 0;
        (*g_0257ef48)();
        puVar9 = (void*)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510);
        if (puVar9 == puVar7) {
          bVar3 = false;
        }
        else {
          g_028b7510._0_7_ = SUB87(puVar7,0);
          g_028b7510._7_1_ = (uint8_t)((uint64_t)puVar7 >> 0x38);
          bVar3 = true;
          if (puVar9 != (void*)0x0) {
            FUN_00d50b20();
          }
        }
        if (g_028b7518 == '\0') {
          g_028b7518 = '\x01';
          FUN_00e8cb90();
        }
        if (!bVar3) {
          FUN_00d50b20();
        }
        g_028b7519 = '\x01';
        fVar11 = (float)FUN_00e8cb70();
      }
      else {
        g_028b7519 = '\x01';
        fVar11 = (float)FUN_00e8cb70();
      }
      plVar2 = (int64_t *)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510);
    }
    if (plVar2 != (int64_t *)0x0) {
      uVar8 = FUN_00e7b4e0();
      FUN_00d8e3d0(extraout_XMM0_Da_00,uVar8);
      local_98 = local_58;
      local_90 = 0;
      if ((char)local_50 == '\0') {
        if (local_58 != (void*)0x0) {
          FUN_00d50b00();
        }
      }
      else {
        local_50 = local_50 & 0xffffff00;
      }
      local_90 = '\x01';
      uVar10 = (**(code **)(*plVar2 + 0x368))();
      if ((local_90 != '\0') && (local_98 != (void*)0x0)) {
        uVar10 = FUN_00d50b20();
      }
      if (((char)local_50 != '\0') && (local_58 != (void*)0x0)) {
        uVar10 = FUN_00d50b20();
      }
      cVar5 = (**(code **)(*(int64_t *)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510) +
                          0x430))(uVar10,2);
      lVar4 = g_027eec90;
      if (cVar5 == '\0') {
        uVar10 = extraout_XMM0_Da_01;
        if (g_027eec90 != 0) {
          uVar10 = FUN_00d50b00();
        }
        FUN_00cc7b40(uVar10,g_025908a0);
        if (lVar4 != 0) {
          FUN_00d50b20();
        }
      }
      plVar2 = (int64_t *)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510);
      uVar8 = FUN_00e7b4e0();
      FUN_00d8e3d0(extraout_XMM0_Da_02,uVar8);
      local_88 = local_58;
      local_80 = 0;
      if ((char)local_50 == '\0') {
        if (local_58 != (void*)0x0) {
          FUN_00d50b00();
        }
      }
      else {
        local_50 = local_50 & 0xffffff00;
      }
      local_80 = '\x01';
      uVar10 = (**(code **)(*plVar2 + 0x368))();
      if ((local_80 != '\0') && (local_88 != (void*)0x0)) {
        uVar10 = FUN_00d50b20();
      }
      if (((char)local_50 != '\0') && (local_58 != (void*)0x0)) {
        uVar10 = FUN_00d50b20();
      }
      cVar5 = (**(code **)(*(int64_t *)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510) +
                          0x430))(uVar10,2);
      lVar4 = g_027eec90;
      if (cVar5 == '\0') {
        uVar10 = extraout_XMM0_Da_03;
        if (g_027eec90 != 0) {
          uVar10 = FUN_00d50b00();
        }
        FUN_00cc7b40(uVar10,g_025908a0);
        if (lVar4 != 0) {
          FUN_00d50b20();
        }
      }
      plVar2 = (int64_t *)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510);
      uVar8 = FUN_00e7b4e0();
      FUN_00d8e3d0(extraout_XMM0_Da_04,uVar8);
      local_78 = local_58;
      local_70 = 0;
      if ((char)local_50 == '\0') {
        if (local_58 != (void*)0x0) {
          FUN_00d50b00();
        }
      }
      else {
        local_50 = local_50 & 0xffffff00;
      }
      local_70 = '\x01';
      uVar10 = (**(code **)(*plVar2 + 0x368))();
      if ((local_70 != '\0') && (local_78 != (void*)0x0)) {
        uVar10 = FUN_00d50b20();
      }
      if (((char)local_50 != '\0') && (local_58 != (void*)0x0)) {
        uVar10 = FUN_00d50b20();
      }
      cVar5 = (**(code **)(*(int64_t *)CONCAT17(g_028b7510._7_1_,(undefined7)g_028b7510) +
                          0x430))(uVar10,2);
      lVar4 = g_027eec90;
      fVar11 = extraout_XMM0_Da_05;
      if (cVar5 == '\0') {
        if (g_027eec90 != 0) {
          fVar11 = (float)FUN_00d50b00();
        }
        fVar11 = (float)FUN_00cc7b40(fVar11,g_025908a0);
        if (lVar4 != 0) {
          fVar11 = (float)FUN_00d50b20();
        }
      }
    }
    iVar6 = 0xff;
    bVar3 = false;
    puVar9 = (void*)0x0;
  }
  else {
    if (g_0276cc80 != 0) {
      FUN_00d50b00();
    }
    local_a8 = lVar4;
    local_a0 = '\x01';
    uVar10 = FUN_00d91000(1,&local_a8);
    puVar9 = local_58;
    if (local_58 == (void*)0x0) {
      if ((local_a0 != '\0') && (local_a8 != 0)) {
        uVar10 = FUN_00d50b20();
      }
      puVar9 = (void*)0x0;
      bVar3 = false;
      lVar4 = g_027eec90;
    }
    else {
      if ((char)local_50 == '\0') {
        uVar10 = FUN_00d50b00();
      }
      else {
        local_50 = local_50 & 0xffffff00;
      }
      iVar6 = *(int *)((int64_t)puVar9 + 0xc);
      if (((char)local_50 != '\0') && (local_58 != (void*)0x0)) {
        uVar10 = FUN_00d50b20();
      }
      if ((local_a0 != '\0') && (local_a8 != 0)) {
        uVar10 = FUN_00d50b20();
      }
      if (2 < iVar6) {
        fVar11 = (float)FUN_00d8d7b0();
        local_64 = (uint)(fVar11 * g_02394220);
        fVar11 = (float)FUN_00d8d7b0();
        local_60 = (int)(fVar11 * g_02394220);
        fVar11 = (float)FUN_00d8d7b0();
        fVar11 = fVar11 * g_02394220;
        local_5c = (int)fVar11;
        bVar3 = true;
        if (*(int *)((int64_t)puVar9 + 0xc) == 4) {
          fVar11 = (float)FUN_00d8d7b0();
          fVar11 = fVar11 * g_02394220;
          iVar6 = (int)fVar11;
        }
        else {
          iVar6 = 0xff;
        }
        goto LAB_01cd2460;
      }
      bVar3 = true;
      lVar4 = g_027eec90;
    }
    g_027eec90 = lVar4;
    if (lVar4 != 0) {
      uVar10 = FUN_00d50b00();
    }
    lVar1 = *param_2;
    local_50 = 1;
    local_58 = &g_024c5048;
    local_40 = 0;
    if (lVar1 != 0) {
      uVar10 = FUN_00d50b00();
    }
    local_40 = '\x01';
    local_48 = lVar1;
    fVar11 = (float)FUN_00cc7b40(uVar10,&local_58);
    local_58 = &g_024c5048;
    if ((local_40 != '\0') && (local_48 != 0)) {
      fVar11 = (float)FUN_00d50b20();
    }
    iVar6 = 0xff;
    if (lVar4 != 0) {
      fVar11 = (float)FUN_00d50b20();
    }
  }
LAB_01cd2460:
  lVar4 = g_027eec90;
  if (((((0xff < local_64) || (local_60 < 0)) || (0xff < local_60)) ||
      ((local_5c < 0 || (0xff < local_5c)))) || ((iVar6 < 0 || (0xff < iVar6)))) {
    if (g_027eec90 != 0) {
      fVar11 = (float)FUN_00d50b00();
    }
    lVar1 = *param_2;
    local_50 = 1;
    local_58 = &g_024c5048;
    if (lVar1 != 0) {
      fVar11 = (float)FUN_00d50b00();
    }
    local_40 = '\x01';
    local_48 = lVar1;
    FUN_00cc7b40(fVar11,&local_58);
    local_58 = &g_024c5048;
    if ((local_40 != '\0') && (local_48 != 0)) {
      FUN_00d50b20();
    }
    if (lVar4 != 0) {
      FUN_00d50b20();
    }
  }
  FUN_01cfcdc0((float)(int)local_64 / g_02394220,(float)local_60 / g_02394220,
               (float)local_5c / g_02394220,(float)iVar6 / g_02394220);
  if ((bVar3) && (puVar9 != (void*)0x0)) {
    FUN_00d50b20();
  }
  return this_ptr;
}

