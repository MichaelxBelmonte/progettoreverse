// Function: FUN_01f4eb70
// Address: 01f4eb70
// Size: 2137 bytes
// Class: GNString
// === GNString properties ===
//   bool            _dragEnabled
//   bool            _dropEnabled
//   double          _transferStartTime
//   double          _transferEndTime
//   double          _userStartTime
//   double          _userEndTime


bool FUN_01f4eb70(void)

{
  void*puVar1;
  char cVar2;
  int64_t lVar3;
  int64_t lVar4;
  uint64_t uVar5;
  code *pcVar6;
  int64_t this_ptr;
  uint64_t uVar7;
  int64_t *local_50;
  char local_48;
  int64_t local_40;
  char local_38;
  
  _objc_alloc();
  puVar1 = PTR__objc_msgSend_024a9998;
  (*PTR__objc_msgSend_024a9998)();
  FUN_00d216c0();
  (*puVar1)();
  (*puVar1)();
  (*puVar1)();
  (*puVar1)();
  (*puVar1)();
  lVar3 = *(int64_t *)(this_ptr + 0x30);
  if (lVar3 != 0) {
    FUN_00d50b00();
    FUN_00e1cfc0();
    (*PTR__objc_msgSend_024a9998)();
    if (lVar3 != 0) {
      FUN_00d50b20();
    }
  }
  FUN_01f4fff0();
  if ((local_38 != '\0') && (local_40 != 0)) {
    FUN_00d50b20();
  }
  if (local_40 != 0) {
    FUN_01f4fff0();
    if (local_38 == '\0') {
      if (local_40 != 0) {
        FUN_00d50b00();
      }
    }
    else {
      local_38 = '\0';
    }
    FUN_00e1cfc0();
    (*PTR__objc_msgSend_024a9998)();
    if (local_40 != 0) {
      FUN_00d50b20();
    }
    if ((local_38 != '\0') && (local_40 != 0)) {
      FUN_00d50b20();
    }
  }
  pcVar6 = PTR__objc_msgSend_024a9998;
  if (*(int64_t *)(this_ptr + 0x10) != 0) {
    _objc_alloc();
    lVar3 = *(int64_t *)(this_ptr + 0x10);
    if (lVar3 != 0) {
      FUN_00d50b00();
    }
    (*PTR__objc_msgSend_024a9998)();
    (*PTR__objc_msgSend_024a9998)();
    pcVar6 = PTR__objc_msgSend_024a9998;
    if (lVar3 != 0) {
      FUN_00d50b20();
    }
  }
  if (*(int64_t **)(this_ptr + 0x28) == (int64_t *)0x0) {
LAB_01f4f0b8:
    (*PTR__objc_msgSend_024a9998)();
  }
  else {
    cVar2 = (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x398))();
    if (cVar2 == '\0') {
      cVar2 = (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x3a0))();
      if (cVar2 == '\0') {
        (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x3f0))();
        if (local_40 == 0) {
          cVar2 = '\0';
        }
        else {
          (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x3f0))();
          cVar2 = (**(code **)(*local_50 + 0x3a0))();
          if ((local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
            FUN_00d50b20();
          }
        }
        if ((local_38 != '\0') && (local_40 != 0)) {
          FUN_00d50b20();
        }
        pcVar6 = PTR__objc_msgSend_024a9998;
        if (cVar2 == '\0') goto LAB_01f4f0b8;
        (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x3f0))();
        (**(code **)(*local_50 + 0x368))();
        if (local_38 == '\0') {
          if (local_40 != 0) {
            FUN_00d50b00();
          }
        }
        else {
          local_38 = '\0';
        }
        lVar3 = FUN_00e1cfc0();
        pcVar6 = PTR__objc_msgSend_024a9998;
        if (local_40 != 0) {
          FUN_00d50b20();
        }
        if ((local_38 != '\0') && (local_40 != 0)) {
          FUN_00d50b20();
        }
        if ((local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
          FUN_00d50b20();
        }
      }
      else {
        (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x368))();
        if (local_38 == '\0') {
          if (local_40 != 0) {
            FUN_00d50b00();
          }
        }
        else {
          local_38 = '\0';
        }
        lVar3 = FUN_00e1cfc0();
        pcVar6 = PTR__objc_msgSend_024a9998;
        if (local_40 != 0) {
          FUN_00d50b20();
        }
        if ((local_38 != '\0') && (local_40 != 0)) {
          FUN_00d50b20();
        }
      }
    }
    else {
      (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x368))();
      FUN_00d8a060();
      if (local_38 == '\0') {
        if (local_40 != 0) {
          FUN_00d50b00();
        }
      }
      else {
        local_38 = '\0';
      }
      FUN_00e1cfc0();
      if (local_40 != 0) {
        FUN_00d50b20();
      }
      if ((local_38 != '\0') && (local_40 != 0)) {
        FUN_00d50b20();
      }
      if ((local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
        FUN_00d50b20();
      }
      (**(code **)(**(int64_t **)(this_ptr + 0x28) + 0x3f0))();
      (**(code **)(*local_50 + 0x368))();
      if (local_38 == '\0') {
        if (local_40 != 0) {
          FUN_00d50b00();
        }
      }
      else {
        local_38 = '\0';
      }
      lVar3 = FUN_00e1cfc0();
      pcVar6 = PTR__objc_msgSend_024a9998;
      if (local_40 != 0) {
        FUN_00d50b20();
      }
      if ((local_38 != '\0') && (local_40 != 0)) {
        FUN_00d50b20();
      }
      if ((local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
        FUN_00d50b20();
      }
    }
    (*PTR__objc_msgSend_024a9998)();
    if (lVar3 != 0) {
      (*PTR__objc_msgSend_024a9998)(1,lVar3);
    }
  }
  (*pcVar6)();
  (*pcVar6)();
  _objc_alloc();
  (*pcVar6)();
  _objc_autorelease();
  FUN_00d50b00();
  (*PTR__objc_msgSend_024a9998)();
  if (this_ptr != 0) {
    FUN_00d50b20();
  }
  (*PTR__objc_msgSend_024a9998)();
  FUN_01f27fe0();
  *(int *)(local_40 + 0xdc) = *(int *)(local_40 + 0xdc) + 1;
  if (local_38 != '\0') {
    FUN_00d50b20();
  }
  lVar3 = (*PTR__objc_msgSend_024a9998)();
  FUN_01f27fe0();
  *(int *)(local_40 + 0xdc) = *(int *)(local_40 + 0xdc) + -1;
  if (local_38 != '\0') {
    FUN_00d50b20();
  }
  pcVar6 = PTR__objc_msgSend_024a9998;
  if (lVar3 == 1) {
    (*PTR__objc_msgSend_024a9998)();
    lVar4 = (*pcVar6)();
    if (lVar4 != 0) {
      uVar7 = 0;
      do {
        (*pcVar6)();
        FUN_00e1d1d0();
        if (local_38 == '\0') {
          if (local_40 != 0) {
            FUN_00d50b00();
            goto LAB_01f4f350;
          }
        }
        else if (local_40 != 0) {
LAB_01f4f350:
          local_38 = '\0';
          FUN_00d21140();
          FUN_00d50b20();
        }
        pcVar6 = PTR__objc_msgSend_024a9998;
        uVar7 = uVar7 + 1;
        uVar5 = (*PTR__objc_msgSend_024a9998)();
      } while (uVar7 < uVar5);
    }
  }
  (*PTR__objc_msgSend_024a9998)();
  (*PTR__objc_release_024a99a0)();
  return lVar3 != 1;
}

