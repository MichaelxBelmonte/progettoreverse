// Function: FUN_01e42850
// Address: 01e42850
// Size: 675 bytes
// Class: GNList
// === GNList properties ===
//   GNInt           _archiveVersion
//   bool            _awakesObjectsAfterDecoding
//   bool            _isCancelled
//   GNInt           _currentObjectGraphIdx
//   GNInt           _predictedDecodedValuesIndex


void FUN_01e42850(void)

{
  char cVar1;
  int64_t *this_ptr;
  float fVar2;
  float fVar3;
  uint64_t uVar4;
  uint64_t uVar5;
  uint64_t extraout_XMM0_Qb;
  uint64_t extraout_XMM0_Qb_00;
  uint8_t auVar6 [16];
  uint8_t auVar7 [16];
  float local_88;
  float fStack_84;
  float fStack_80;
  float fStack_7c;
  float local_78;
  float fStack_74;
  float fStack_70;
  float fStack_6c;
  int64_t *local_50;
  char local_48;
  int64_t *in_stack_ffffffffffffffc0;
  char local_38;
  
  (**(code **)(*this_ptr + 0x7b8))();
  if (local_38 == '\0') {
    if (in_stack_ffffffffffffffc0 == (int64_t *)0x0) {
      return;
    }
    FUN_00d50b00();
  }
  else if (in_stack_ffffffffffffffc0 == (int64_t *)0x0) {
    return;
  }
  FUN_01d97850();
  if (local_38 == '\0') {
    if (in_stack_ffffffffffffffc0 == (int64_t *)0x0) goto LAB_01e42ad8;
    FUN_00d50b00();
  }
  else if (in_stack_ffffffffffffffc0 == (int64_t *)0x0) goto LAB_01e42ad8;
  FUN_01d99a50();
  FUN_00d50b00();
  cVar1 = FUN_00d24090();
  if (cVar1 == '\0') {
    FUN_01d982e0();
  }
  FUN_00d50b20();
  if ((local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  (**(code **)(*in_stack_ffffffffffffffc0 + 0x7b0))();
  fVar2 = (float)(**(code **)(*this_ptr + 0x4d8))();
  (**(code **)(*this_ptr + 0x7b0))();
  fVar3 = (float)(**(code **)(*local_50 + 0x4d8))();
  if ((local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  auVar6 = blendps(ZEXT416(0),ZEXT416((uint)((fVar2 - fVar3) + 0.0)),1);
  FUN_01d98320();
  (**(code **)(*in_stack_ffffffffffffffc0 + 0x7b0))();
  uVar4 = (**(code **)(*this_ptr + 0x4d8))();
  (**(code **)(*this_ptr + 0x7b0))();
  uVar5 = (**(code **)(*local_50 + 0x4d8))();
  if ((local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  local_78 = (float)uVar4;
  fStack_74 = (float)((uint64_t)uVar4 >> 0x20);
  fStack_70 = (float)extraout_XMM0_Qb;
  fStack_6c = (float)((uint64_t)extraout_XMM0_Qb >> 0x20);
  local_88 = (float)uVar5;
  fStack_84 = (float)((uint64_t)uVar5 >> 0x20);
  fStack_80 = (float)extraout_XMM0_Qb_00;
  fStack_7c = (float)((uint64_t)extraout_XMM0_Qb_00 >> 0x20);
  auVar7._0_4_ = (local_78 - local_88) + auVar6._0_4_;
  auVar7._4_4_ = (fStack_74 - fStack_84) + auVar6._4_4_;
  auVar7._8_4_ = (fStack_70 - fStack_80) + auVar6._8_4_;
  auVar7._12_4_ = (fStack_6c - fStack_7c) + auVar6._12_4_;
  blendps(auVar6,auVar7,2);
  FUN_00d50b20();
LAB_01e42ad8:
  FUN_00d50b20();
  return;
}

