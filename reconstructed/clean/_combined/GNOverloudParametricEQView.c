// ===================================================================
// GNOverloudParametricEQView — Complete reconstructed pseudocode
// 3 functions
// ===================================================================


// ============================================================
// 00bfcee0
// ============================================================
// Function: FUN_00bfcee0
// Address: 00bfcee0
// Size: 4757 bytes
// Class: GNOverloudParametricEQView
// String references:
//   "%I"
//   "%Ik"

void FUN_00bfcee0(void)

{
  int iVar1;
  uint8_t auVar2 [12];
  uint8_t auVar3 [16];
  uint8_t auVar4 [16];
  bool bVar5;
  uint8_t auVar6 [16];
  code *pcVar7;
  int64_t *plVar8;
  int iVar9;
  void*puVar10;
  void*puVar11;
  void*puVar12;
  int iVar13;
  int64_t lVar14;
  int64_t *plVar15;
  uint64_t uVar16;
  void*arg1;
  int64_t *this_ptr;
  uint uVar17;
  uint64_t uVar18;
  float fVar19;
  float fVar20;
  float fVar21;
  uint64_t uVar22;
  uint64_t uVar23;
  float extraout_XMM0_Db;
  uint64_t extraout_XMM0_Qb;
  uint64_t extraout_XMM0_Qb_00;
  uint8_t auVar24 [16];
  uint8_t auVar25 [16];
  float extraout_XMM0_Db_00;
  uint64_t extraout_XMM0_Qb_01;
  uint64_t extraout_XMM0_Qb_02;
  uint8_t auVar26 [16];
  uint8_t auVar27 [16];
  uint8_t auVar28 [16];
  uint8_t auVar29 [16];
  uint8_t auVar30 [16];
  float extraout_XMM0_Db_01;
  uint64_t extraout_XMM0_Qb_03;
  uint8_t auVar31 [16];
  uint8_t auVar32 [16];
  float extraout_XMM0_Db_02;
  uint8_t auVar33 [16];
  uint8_t auVar34 [16];
  float fVar35;
  uint8_t in_XMM1 [16];
  uint8_t auVar36 [16];
  uint8_t auVar37 [16];
  uint8_t auVar38 [16];
  uint8_t auVar39 [16];
  uint8_t auVar40 [16];
  uint8_t auVar41 [16];
  uint8_t auVar42 [16];
  uint8_t auVar43 [16];
  uint8_t auVar44 [16];
  uint8_t auVar45 [16];
  uint8_t auVar46 [16];
  uint8_t auVar47 [16];
  uint8_t auVar48 [16];
  uint8_t auVar49 [16];
  int64_t *local_1b8;
  char local_1b0;
  uint32_t uStack_1a0;
  uint32_t uStack_19c;
  float local_138;
  float fStack_134;
  float fStack_130;
  float fStack_12c;
  float local_118;
  float fStack_114;
  float fStack_110;
  float fStack_10c;
  float local_108;
  float fStack_104;
  float fStack_100;
  float fStack_fc;
  uint8_t local_98 [8];
  float fStack_90;
  float fStack_8c;
  float local_78;
  uint uStack_74;
  uint uStack_70;
  uint uStack_6c;
  int64_t *local_50;
  uint32_t local_48;
  int local_44;
  int64_t *local_40;
  char local_38;
  uint64_t extraout_XMM0_Qb_04;
  
  (**(code **)(*this_ptr + 0x640))();
  plVar15 = local_40;
  FUN_01e3f820();
  (**(code **)(*plVar15 + 0x3f0))(8,0x11,0,1);
  if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  uVar22 = FUN_00bfce40();
  auVar24 = in_XMM1;
  iVar9 = FUN_00e7d780();
  puVar10 = (void*)FUN_00e8fc40();
  FUN_00d4ff40();
  *puVar10 = &g_025683c0;
  pcVar7 = g_025683d8;
  (*g_025683d8)();
  FUN_00c92170();
  uVar23 = FUN_00c92160();
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x41a00000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x42700000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x42c80000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x43480000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x43fa0000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x447a0000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x44fa0000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x459c4000;
  iVar1 = *(int *)(puVar10 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x461c4000;
  iVar1 = *(int *)(puVar10 + 3);
  FUN_00c8e340(uVar23,1);
  *(void*)(puVar10[2] + (int64_t)iVar1) = 0x469c4000;
  puVar11 = (void*)FUN_00e8fc40();
  FUN_00d4ff40();
  *puVar11 = &g_025683c0;
  (*pcVar7)();
  FUN_00c92170();
  uVar23 = FUN_00c92160();
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0xc1f00000;
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0xc1900000;
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0xc1400000;
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0xc0c00000;
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0;
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0x40c00000;
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0x41400000;
  iVar1 = *(int *)(puVar11 + 3);
  uVar23 = FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0x41900000;
  iVar1 = *(int *)(puVar11 + 3);
  FUN_00c8e340(uVar23,1);
  *(void*)(puVar11[2] + (int64_t)iVar1) = 0x41f00000;
  FUN_01d48b40();
  (**(code **)(*this_ptr + 0x640))();
  (**(code **)(*local_50 + 0x3a8))();
  plVar15 = local_40;
  if (local_38 == '\0') {
    if (local_40 != (int64_t *)0x0) {
      FUN_00d50b00();
    }
  }
  else {
    local_38 = '\0';
  }
  FUN_01d488d0();
  if (plVar15 != (int64_t *)0x0) {
    FUN_00d50b20();
  }
  if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  if (((char)local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  FUN_01d489d0();
  local_138 = (float)uVar22;
  fStack_134 = (float)((uint64_t)uVar22 >> 0x20);
  fStack_130 = (float)extraout_XMM0_Qb;
  fStack_12c = (float)((uint64_t)extraout_XMM0_Qb >> 0x20);
  local_108 = in_XMM1._0_4_;
  fStack_104 = in_XMM1._4_4_;
  fStack_100 = in_XMM1._8_4_;
  fStack_fc = in_XMM1._12_4_;
  fStack_114 = fStack_134 + fStack_104;
  local_118 = local_138 + local_108;
  fStack_110 = fStack_130 + fStack_100;
  fStack_10c = fStack_12c + fStack_fc;
  if (3 < *(int *)(puVar10 + 3)) {
    local_98._4_4_ = fStack_134;
    local_98._0_4_ = fStack_134;
    fStack_90 = fStack_12c;
    fStack_8c = fStack_12c;
    lVar14 = 0;
    auVar49 = auVar24;
    do {
      FUN_00bfce40();
      auVar24 = auVar49;
      uVar22 = _logf();
      local_78 = auVar49._0_4_;
      fVar19 = (((float)uVar22 / g_0241c6f0 + g_023d9584) * local_78) / g_023d9588;
      if (lVar14 != 0) {
        iVar1 = *(int *)(puVar10 + 3);
        iVar13 = iVar1 + 3;
        if (-1 < iVar1) {
          iVar13 = iVar1;
        }
        if (lVar14 < (iVar13 >> 2) + -1) {
          auVar36._0_4_ = (uint)fVar19 & g_023945e0;
          auVar36._4_4_ = (uint)((uint64_t)uVar22 >> 0x20) & _UNK_023945e4;
          auVar36._8_4_ = (uint)extraout_XMM0_Qb_00 & _UNK_023945e8;
          auVar36._12_4_ = (uint)((uint64_t)extraout_XMM0_Qb_00 >> 0x20) & _UNK_023945ec;
          auVar37._4_12_ = SUB1612(auVar36 | g_023945f0,4);
          auVar37._0_4_ = SUB164(auVar36 | g_023945f0,0) + fVar19;
          auVar49._0_12_ = ZEXT812(0);
          auVar49._12_4_ = 0;
          roundss(auVar49,auVar37,0xb);
          auVar24 = _local_98;
          (**(code **)(*(int64_t *)*arg1 + 0x3e0))();
        }
      }
      lVar14 = lVar14 + 1;
      iVar1 = *(int *)(puVar10 + 3);
      iVar13 = iVar1 + 3;
      if (-1 < iVar1) {
        iVar13 = iVar1;
      }
      auVar49 = auVar24;
    } while (lVar14 < iVar13 >> 2);
  }
  auVar32._4_12_ = local_118._4_12_;
  fVar19 = local_138 + local_108 + g_02390d00;
  if (3 < *(int *)(puVar11 + 3)) {
    lVar14 = 0;
    do {
      fVar21 = *(float *)(puVar11[2] + lVar14 * 4);
      FUN_00bfce40();
      fVar20 = auVar24._4_4_ * g_0239011c;
      fVar21 = fVar21 / g_023b36a8;
      fVar35 = *(float *)(puVar11[2] + lVar14 * 4);
      if ((fVar35 != g_0239424c) || (NAN(fVar35) || NAN(g_0239424c))) {
        (**(code **)(*this_ptr + 0x640))();
        (**(code **)(*local_50 + 0x3a8))();
        plVar15 = local_40;
        if (local_38 == '\0') {
          if (local_40 != (int64_t *)0x0) {
            FUN_00d50b00();
          }
        }
        else {
          local_38 = '\0';
        }
        FUN_01d488d0();
        if (plVar15 != (int64_t *)0x0) {
          FUN_00d50b20();
        }
        if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
          FUN_00d50b20();
        }
        if (((char)local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
          FUN_00d50b20();
        }
      }
      else {
        (**(code **)(*this_ptr + 0x640))();
        (**(code **)(*local_50 + 0x390))();
        plVar15 = local_40;
        if (local_38 == '\0') {
          if (local_40 != (int64_t *)0x0) {
            FUN_00d50b00();
          }
        }
        else {
          local_38 = '\0';
        }
        FUN_01d488d0();
        if (plVar15 != (int64_t *)0x0) {
          FUN_00d50b20();
        }
        if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
          FUN_00d50b20();
        }
        if (((char)local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
          FUN_00d50b20();
        }
      }
      if (lVar14 != 0) {
        iVar1 = *(int *)(puVar11 + 3);
        iVar13 = iVar1 + 3;
        if (-1 < iVar1) {
          iVar13 = iVar1;
        }
        if (lVar14 < (iVar13 >> 2) + -1) {
          fVar21 = fVar21 * fVar20 + extraout_XMM0_Db + fVar20;
          auVar24 = ZEXT416((uint)fVar21 & g_023945e0) | g_023945f0;
          auVar25._4_12_ = auVar24._4_12_;
          auVar25._0_4_ = auVar24._0_4_ + fVar21;
          auVar38._0_12_ = ZEXT812(0);
          auVar38._12_4_ = 0;
          auVar24 = roundss(auVar38,auVar25,0xb);
          (**(code **)(*(int64_t *)*arg1 + 0x3e0))();
        }
      }
      lVar14 = lVar14 + 1;
      iVar1 = *(int *)(puVar11 + 3);
      iVar13 = iVar1 + 3;
      if (-1 < iVar1) {
        iVar13 = iVar1;
      }
    } while (lVar14 < iVar13 >> 2);
  }
  (**(code **)(*this_ptr + 0x640))();
  (**(code **)(*local_50 + 0x390))();
  plVar15 = local_40;
  if (local_38 == '\0') {
    if (local_40 != (int64_t *)0x0) {
      FUN_00d50b00();
    }
  }
  else {
    local_38 = '\0';
  }
  FUN_01d488d0();
  if (plVar15 != (int64_t *)0x0) {
    FUN_00d50b20();
  }
  if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  if (((char)local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  FUN_01d489d0();
  (**(code **)(*this_ptr + 0x640))();
  (**(code **)(*local_1b8 + 0x370))();
  FUN_01d44d80();
  plVar15 = local_40;
  if (local_38 == '\0') {
    if (local_40 != (int64_t *)0x0) {
      FUN_00d50b00();
    }
  }
  else {
    local_38 = '\0';
  }
  FUN_01d48a10();
  if (plVar15 != (int64_t *)0x0) {
    FUN_00d50b20();
  }
  if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  if (((char)local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  if ((local_1b0 != '\0') && (local_1b8 != (int64_t *)0x0)) {
    FUN_00d50b20();
  }
  if (3 < *(int *)(puVar10 + 3)) {
    lVar14 = 0;
    do {
      fVar21 = *(float *)(puVar10[2] + lVar14 * 4);
      if (g_02393944 <= fVar21) {
        local_44 = (int)fVar21 / 1000;
        local_48 = 1;
        local_50 = &g_024cc6f0;
        FUN_00d8cb40(&g_024cc6f0,&local_50);
        plVar15 = local_40;
        if (local_40 == (int64_t *)0x0) goto LAB_00bfda60;
        bVar5 = true;
        if (((local_38 == '\0') && (FUN_00d50b00(), local_38 != '\0')) &&
           (local_40 != (int64_t *)0x0)) {
          FUN_00d50b20();
        }
      }
      else {
        local_44 = (int)fVar21;
        local_48 = 1;
        local_50 = &g_024cc6f0;
        FUN_00d8cb40(&g_024cc6f0,&local_50);
        plVar15 = local_40;
        if (local_40 == (int64_t *)0x0) {
LAB_00bfda60:
          bVar5 = false;
          plVar15 = local_40;
        }
        else {
          bVar5 = true;
          if (((local_38 == '\0') && (FUN_00d50b00(), local_38 != '\0')) &&
             (local_40 != (int64_t *)0x0)) {
            FUN_00d50b20();
          }
        }
      }
      FUN_00bfce40();
      auVar39._4_4_ = auVar24._4_4_;
      auVar39._0_4_ = auVar39._4_4_;
      auVar39._12_4_ = auVar24._12_4_;
      auVar39._8_4_ = auVar39._12_4_;
      auVar40._4_12_ = auVar39._4_12_;
      auVar40._0_4_ =
           auVar39._4_4_ * g_0239011c * g_0241e984 +
           extraout_XMM0_Db_00 + auVar39._4_4_ * g_0239011c + g_023b36b0 + g_02390d34;
      FUN_00bfce40();
      uVar22 = _logf();
      local_78 = auVar40._0_4_;
      fVar21 = (((float)uVar22 / g_0241c6f0 + g_023d9584) * local_78) / g_023d9588;
      FUN_01d48ad0();
      uVar23 = FUN_01d43d10();
      if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
        FUN_00d50b20();
      }
      uStack_74 = (uint)((uint64_t)uVar22 >> 0x20);
      uStack_70 = (uint)extraout_XMM0_Qb_01;
      uStack_6c = (uint)((uint64_t)extraout_XMM0_Qb_01 >> 0x20);
      auVar26._0_4_ = (uint)fVar21 & g_023945e0;
      auVar26._4_4_ = uStack_74 & _UNK_023945e4;
      auVar26._8_4_ = uStack_70 & _UNK_023945e8;
      auVar26._12_4_ = uStack_6c & _UNK_023945ec;
      auVar27._4_12_ = SUB1612(auVar26 | g_023945f0,4);
      auVar27._0_4_ = SUB164(auVar26 | g_023945f0,0) + fVar21;
      auVar24 = roundss(auVar27,auVar27,0xb);
      auVar4._8_8_ = extraout_XMM0_Qb_02;
      auVar4._0_8_ = uVar23;
      auVar41 = roundss(auVar4,auVar4,10);
      iVar1 = *(int *)(puVar10 + 3);
      iVar13 = iVar1 + 3;
      if (-1 < iVar1) {
        iVar13 = iVar1;
      }
      fVar21 = auVar24._0_4_;
      auVar28._4_12_ = auVar24._4_12_;
      fVar35 = auVar41._0_4_;
      if (lVar14 < (iVar13 >> 2) + -1) {
        if (lVar14 == 0) {
          auVar30._0_4_ = fVar21 + g_02390d34;
          auVar30._4_12_ = auVar28._4_12_;
          uVar22 = auVar30._0_8_;
        }
        else {
          auVar45._0_4_ = (uint)(fVar35 * g_0239011c) & g_023945e0;
          auVar45._4_4_ = auVar41._4_4_ & _UNK_023945e4;
          auVar45._8_4_ = auVar41._8_4_ & _UNK_023945e8;
          auVar45._12_4_ = auVar41._12_4_ & _UNK_023945ec;
          auVar46._4_12_ = SUB1612(auVar45 | g_023945f0,4);
          auVar46._0_4_ = SUB164(auVar45 | g_023945f0,0) + fVar35 * g_0239011c;
          auVar42._0_12_ = ZEXT812(0);
          auVar42._12_4_ = 0;
          auVar41 = roundss(auVar42,auVar46,0xb);
          auVar28._0_4_ = fVar21 - auVar41._0_4_;
          uVar22 = auVar28._0_8_;
        }
      }
      else {
        auVar47._0_4_ = (uint)fVar35 & g_023945e0;
        auVar47._4_4_ = auVar41._4_4_ & _UNK_023945e4;
        auVar47._8_4_ = auVar41._8_4_ & _UNK_023945e8;
        auVar47._12_4_ = auVar41._12_4_ & _UNK_023945ec;
        auVar48._4_12_ = SUB1612(auVar47 | g_023945f0,4);
        auVar48._0_4_ = SUB164(auVar47 | g_023945f0,0) + fVar35;
        auVar24 = roundss(ZEXT816(0),auVar48,0xb);
        auVar41._4_12_ = auVar24._4_12_;
        auVar41._0_4_ = auVar24._0_4_ + g_02390d34;
        auVar29._0_4_ = fVar21 - auVar41._0_4_;
        auVar29._4_12_ = auVar28._4_12_;
        uVar22 = auVar29._0_8_;
      }
      auVar2._4_8_ = auVar41._8_8_;
      auVar2._0_4_ = auVar39._4_4_;
      auVar24._0_8_ = auVar2._0_8_ << 0x20;
      auVar24._8_4_ = auVar39._12_4_;
      auVar24._12_4_ = auVar39._12_4_;
      (**(code **)(*(int64_t *)*arg1 + 0x3f0))(uVar22);
      if ((bVar5) && (plVar15 != (int64_t *)0x0)) {
        FUN_00d50b20();
      }
      lVar14 = lVar14 + 1;
      iVar1 = *(int *)(puVar10 + 3);
      iVar13 = iVar1 + 3;
      if (-1 < iVar1) {
        iVar13 = iVar1;
      }
    } while (lVar14 < iVar13 >> 2);
  }
  if (3 < *(int *)(puVar11 + 3)) {
    lVar14 = 0;
    do {
      fVar21 = *(float *)(puVar11[2] + lVar14 * 4);
      FUN_00bfce40();
      auVar43._4_4_ = auVar24._4_4_;
      auVar43._0_4_ = auVar43._4_4_;
      auVar43._12_4_ = auVar24._12_4_;
      auVar43._8_4_ = auVar43._12_4_;
      auVar24._4_12_ = auVar43._4_12_;
      auVar24._0_4_ = auVar43._4_4_ * g_0239011c;
      fVar21 = fVar21 / g_023b36a8;
      fVar35 = *(float *)(puVar11[2] + lVar14 * 4);
      if ((fVar35 != g_023b8b2c) || (NAN(fVar35) || NAN(g_023b8b2c))) {
        local_44 = (int)(float)((uint)fVar35 & g_02390140);
        local_48 = 1;
        local_50 = &g_024cc6f0;
        FUN_00d8cb40(&g_024cc6f0,&local_50);
        plVar15 = local_40;
        if ((local_38 == '\0') &&
           (((local_40 != (int64_t *)0x0 && (FUN_00d50b00(), local_38 != '\0')) &&
            (local_40 != (int64_t *)0x0)))) {
          FUN_00d50b20();
        }
        FUN_01d48ad0();
        uVar22 = FUN_01d43d10();
        uStack_1a0 = (uint32_t)extraout_XMM0_Qb_03;
        uStack_19c = (uint32_t)((uint64_t)extraout_XMM0_Qb_03 >> 0x20);
        if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
          FUN_00d50b20();
        }
        fVar21 = fVar21 * auVar24._0_4_ + extraout_XMM0_Db_01 + auVar24._0_4_;
        auVar24 = ZEXT416((uint)fVar21 & g_023945e0) | g_023945f0;
        auVar31._4_12_ = auVar24._4_12_;
        auVar31._0_4_ = auVar24._0_4_ + fVar21;
        auVar44._0_12_ = ZEXT812(0);
        auVar44._12_4_ = 0;
        auVar24 = roundss(auVar44,auVar31,0xb);
        auVar3._8_4_ = uStack_1a0;
        auVar3._0_8_ = uVar22;
        auVar3._12_4_ = uStack_19c;
        auVar49 = roundss(ZEXT816(0),auVar3,10);
        auVar32._0_4_ = fVar19 - (auVar49._0_4_ + g_02390d34);
        auVar6._12_4_ = 0;
        auVar6._0_12_ = auVar24._4_12_;
        auVar24 = auVar6 << 0x20;
        (**(code **)(*(int64_t *)*arg1 + 0x3f0))(auVar32._0_8_);
        if (plVar15 != (int64_t *)0x0) {
          FUN_00d50b20();
        }
      }
      lVar14 = lVar14 + 1;
      iVar1 = *(int *)(puVar11 + 3);
      iVar13 = iVar1 + 3;
      if (-1 < iVar1) {
        iVar13 = iVar1;
      }
    } while (lVar14 < iVar13 >> 2);
  }
  if (this_ptr[0x27] != 0) {
    FUN_01d48370();
    (**(code **)(*(int64_t *)*arg1 + 0x390))();
    uVar17 = iVar9 - (iVar9 >> 0x1f) & 0xfffffffe;
    (**(code **)(*(int64_t *)this_ptr[0x27] + 0x620))(2,uVar17);
    plVar15 = local_40;
    if (((local_38 == '\0') && (local_40 != (int64_t *)0x0)) &&
       ((FUN_00d50b00(), local_38 != '\0' && (local_40 != (int64_t *)0x0)))) {
      FUN_00d50b20();
    }
    puVar12 = (void*)FUN_00e8fc40();
    FUN_00d4ff40();
    *puVar12 = &g_02680400;
    *(void*)((int64_t)puVar12 + 0xc) = 0;
    puVar12[6] = 0;
    puVar12[7] = 0;
    *(void*)((int64_t)puVar12 + 0x39) = 0;
    *(void*)((int64_t)puVar12 + 0x41) = 0;
    (*g_02680418)();
    if (1 < iVar9) {
      uVar18 = 1;
      if (1 < (int)uVar17) {
        uVar18 = (uint64_t)uVar17;
      }
      uVar16 = 0;
      do {
        lVar14 = plVar15[2];
        if (g_02390124 < *(float *)(lVar14 + uVar16 * 8)) {
          fVar19 = *(float *)(lVar14 + 4 + uVar16 * 8);
          FUN_00bfce40();
          auVar24 = in_XMM1;
          FUN_00bfce40();
          auVar33._0_8_ = _logf();
          auVar33._8_8_ = extraout_XMM0_Qb_04;
          auVar34._4_12_ = auVar33._4_12_;
          local_118 = in_XMM1._0_4_;
          auVar34._0_4_ =
               (((float)auVar33._0_8_ / g_0241c6f0 + g_023d9584) * local_118) / g_023d9588;
          fStack_134 = auVar24._4_4_;
          in_XMM1 = ZEXT416((uint)((fVar19 / g_023b36a8) * fStack_134 * g_0239011c +
                                  extraout_XMM0_Db_02 + fStack_134 * g_0239011c));
          FUN_01d38ba0(auVar34._0_8_);
        }
        uVar16 = uVar16 + 1;
      } while (uVar18 != uVar16);
    }
    (**(code **)(*this_ptr + 0x640))();
    (**(code **)(*local_50 + 0x390))();
    plVar8 = local_40;
    if (local_38 == '\0') {
      if (local_40 != (int64_t *)0x0) {
        FUN_00d50b00();
      }
    }
    else {
      local_38 = '\0';
    }
    FUN_01d488d0();
    if (plVar8 != (int64_t *)0x0) {
      FUN_00d50b20();
    }
    if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
      FUN_00d50b20();
    }
    if (((char)local_48 != '\0') && (local_50 != (int64_t *)0x0)) {
      FUN_00d50b20();
    }
    FUN_01d48b40();
    (**(code **)(*(int64_t *)*arg1 + 0x3a8))();
    FUN_01d48390();
    if (puVar12 != (void*)0x0) {
      FUN_00d50b20();
    }
    if (plVar15 != (int64_t *)0x0) {
      FUN_00d50b20();
    }
    if (puVar11 == (void*)0x0) goto LAB_00bfe191;
  }
  FUN_00d50b20();
LAB_00bfe191:
  if (puVar10 != (void*)0x0) {
    FUN_00d50b20();
  }
  return;
}



// ============================================================
// 00bfc1b0
// ============================================================
// Function: FUN_00bfc1b0
// Address: 00bfc1b0
// Size: 921 bytes
// Class: GNOverloudParametricEQView
// String references:
//   "GNOverloudParametricEQView"

void FUN_00bfc1b0(void)

{
  int64_t *plVar1;
  int64_t *plVar2;
  bool bVar3;
  int64_t lVar4;
  int64_t *plVar5;
  char cVar6;
  int iVar7;
  int64_t *plVar8;
  int64_t **pplVar9;
  uint32_t uVar10;
  uint32_t extraout_XMM0_Da;
  uint32_t extraout_XMM0_Da_00;
  int64_t local_50;
  char local_48;
  int64_t *local_40;
  char local_38;
  
  pplVar9 = &local_40;
  uVar10 = FUN_01ccad60();
  plVar1 = local_40;
  if ((g_0276f570 == '\0') &&
     (iVar7 = ___cxa_guard_acquire(), uVar10 = extraout_XMM0_Da_00, iVar7 != 0)) {
    g_0276f4c0 = FUN_0006d940();
    g_0276f4a8 = "GNOverloudParametricEQView";
    g_0276f4b0 = 0x140;
    g_0276f4b8 = FUN_00c43ad0;
    g_0276f4c8 = 0;
    ram_000000000276f4d0 = 0;
    g_0276f4d8 = 0;
    ram_000000000276f4e0 = 0;
    g_0276f4e8 = 0;
    ram_000000000276f4f0 = 0;
    g_0276f4f8 = 0;
    ram_000000000276f500 = 0;
    g_0276f508 = 0;
    ram_000000000276f510 = 0;
    g_0276f518 = 0;
    ram_000000000276f520 = 0;
    g_0276f528 = 0;
    ram_000000000276f530 = 0;
    g_0276f538 = 0;
    ram_000000000276f540 = 0;
    g_0276f548 = 0;
    ram_000000000276f550 = 0;
    g_0276f558 = 0;
    ram_000000000276f560 = 0;
    g_0276f568 = 0;
    uVar10 = ___cxa_guard_release();
  }
  if (plVar1 == (int64_t *)0x0) {
LAB_00bfc209:
    pplVar9 = (int64_t **)&g_02802688;
  }
  else {
    (**(code **)(*plVar1 + 0x360))();
    cVar6 = FUN_00e85ea0();
    uVar10 = extraout_XMM0_Da;
    if (cVar6 == '\0') goto LAB_00bfc209;
  }
  plVar1 = *pplVar9;
  if (*(char *)(pplVar9 + 1) == '\0') {
    if (plVar1 != (int64_t *)0x0) {
      uVar10 = FUN_00d50b00();
    }
  }
  else {
    *(void*)(pplVar9 + 1) = 0;
  }
  if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
    uVar10 = FUN_00d50b20();
  }
  lVar4 = g_027e7c20;
  if (g_027e7c20 != 0) {
    uVar10 = FUN_00d50b00();
  }
  local_50 = lVar4;
  local_48 = '\x01';
  FUN_01ccaae0(uVar10,&local_50);
  plVar5 = local_40;
  if (local_38 == '\0') {
    if (((local_40 != (int64_t *)0x0) && (FUN_00d50b00(), local_38 != '\0')) &&
       (local_40 != (int64_t *)0x0)) {
      FUN_00d50b20();
    }
  }
  else {
    local_38 = '\0';
  }
  if ((local_48 != '\0') && (local_50 != 0)) {
    FUN_00d50b20();
  }
  FUN_01d26010();
  if (local_40 == (int64_t *)0x0) {
    FUN_00c40480();
LAB_00bfc321:
    bVar3 = true;
  }
  else {
    plVar8 = (int64_t *)(**(code **)(*local_40 + 0x10))();
    FUN_00c40480();
    if (plVar8 == (int64_t *)0x0) goto LAB_00bfc321;
    (**(code **)(*plVar8 + 0x360))();
    cVar6 = FUN_00e85ea0();
    if (cVar6 == '\0') goto LAB_00bfc321;
    FUN_00d50b00();
    bVar3 = false;
  }
  if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
    (**(code **)(*local_40 + 0x10))();
    FUN_00d50b20();
  }
  if (bVar3) goto LAB_00bfc476;
  cVar6 = FUN_00b378e0();
  if (cVar6 == '\0') {
    FUN_00bfc770();
    if (local_38 == '\0') {
      if (local_40 != (int64_t *)0x0) {
        FUN_00d50b00();
        goto LAB_00bfc395;
      }
      if (plVar1[0x27] != 0) {
        plVar1[0x27] = 0;
LAB_00bfc403:
        FUN_00d50b20();
      }
    }
    else {
LAB_00bfc395:
      plVar8 = (int64_t *)plVar1[0x27];
      if (plVar8 != local_40) {
        if (local_40 != (int64_t *)0x0) {
          FUN_00d50b00();
        }
        plVar1[0x27] = (int64_t)local_40;
        if (plVar8 != (int64_t *)0x0) goto LAB_00bfc403;
      }
    }
    (**(code **)(*plVar1 + 0x620))();
    if (local_40 != (int64_t *)0x0) {
      FUN_00d50b20();
    }
  }
  else {
    FUN_00b37610();
    plVar8 = local_40;
    if (local_38 == '\0') {
      if (local_40 != (int64_t *)0x0) {
        FUN_00d50b00();
        goto LAB_00bfc3c9;
      }
      if (plVar1[0x27] != 0) {
        plVar1[0x27] = 0;
        goto LAB_00bfc439;
      }
    }
    else {
      local_38 = '\0';
LAB_00bfc3c9:
      plVar2 = (int64_t *)plVar1[0x27];
      if (plVar2 != plVar8) {
        if (plVar8 != (int64_t *)0x0) {
          FUN_00d50b00();
        }
        plVar1[0x27] = (int64_t)plVar8;
        if (plVar2 != (int64_t *)0x0) {
LAB_00bfc439:
          FUN_00d50b20();
        }
      }
    }
    (**(code **)(*plVar1 + 0x620))();
    if (plVar8 != (int64_t *)0x0) {
      FUN_00d50b20();
    }
    if ((local_38 != '\0') && (local_40 != (int64_t *)0x0)) {
      FUN_00d50b20();
    }
  }
  FUN_00d50b20();
LAB_00bfc476:
  if (plVar5 != (int64_t *)0x0) {
    FUN_00d50b20();
  }
  if (plVar1 != (int64_t *)0x0) {
    FUN_00d50b20();
  }
  return;
}



// ============================================================
// 00bfeab0
// ============================================================
// Function: FUN_00bfeab0
// Address: 00bfeab0
// Size: 990 bytes
// Class: GNOverloudParametricEQView

void FUN_00bfeab0(void)

{
  int64_t lVar1;
  void*puVar2;
  void*puVar3;
  int64_t this_ptr;
  int64_t lVar4;
  bool bVar5;
  int64_t local_c8;
  uint8_t local_c0;
  int64_t local_b8;
  uint8_t local_b0;
  int64_t local_a8;
  uint8_t local_a0;
  int64_t local_98;
  uint8_t local_90;
  int64_t local_88;
  uint8_t local_80;
  int64_t local_78;
  uint8_t local_70;
  int64_t local_68;
  uint8_t local_60;
  int64_t local_58;
  uint8_t local_50;
  int64_t local_48;
  uint8_t local_40;
  int64_t local_38;
  uint8_t local_30;
  
  *(void*)(this_ptr + 0xb8) = 2;
  lVar1 = g_0276bec8;
  if (g_0276bec8 == 0) {
    lVar4 = *(int64_t *)(this_ptr + 0x90);
    if (lVar4 == 0) goto LAB_00bfeb19;
  }
  else {
    FUN_00d50b00();
    lVar4 = *(int64_t *)(this_ptr + 0x90);
    if (lVar4 == lVar1) {
      FUN_00d50b20();
      goto LAB_00bfeb19;
    }
  }
  *(int64_t *)(this_ptr + 0x90) = lVar1;
  if (lVar4 != 0) {
    FUN_00d50b20();
  }
LAB_00bfeb19:
  *(void*)(this_ptr + 0xa8) = 1;
  if ((g_028a5b10 == (void*)0x0) || (g_028a5b19 == '\0')) {
    FUN_00e8cb50();
    if (g_028a5b10 == (void*)0x0) {
      puVar3 = (void*)FUN_00e8fc40();
      FUN_00d4ff40();
      *puVar3 = &g_0254ed40;
      puVar3[2] = 0;
      puVar3[3] = 0;
      puVar3[4] = 0;
      puVar3[5] = 0;
      FUN_00d500e0();
      bVar5 = g_028a5b10 == (void*)0x0;
      g_028a5b10 = puVar3;
      if (((bVar5) || (FUN_00d50b20(), lVar1 = g_0276bed0, g_028a5b10 != (void*)0x0)) &&
         (lVar1 = g_0276bed0, g_028a5b18 == '\0')) {
        g_028a5b18 = '\x01';
        FUN_00e8cb90();
        lVar1 = g_0276bed0;
      }
      g_0276bed0 = lVar1;
      if (lVar1 != 0) {
        FUN_00d50b00();
      }
      lVar4 = g_0276bed8;
      local_c0 = 1;
      local_c8 = lVar1;
      if (g_0276bed8 != 0) {
        FUN_00d50b00();
      }
      local_b8 = lVar4;
      local_b0 = 1;
      FUN_00bf1030(&local_b8,&local_c8);
      if (lVar4 != 0) {
        FUN_00d50b20();
      }
      if (lVar1 != 0) {
        FUN_00d50b20();
      }
      lVar1 = g_0276bee0;
      if (g_0276bee0 != 0) {
        FUN_00d50b00();
      }
      lVar4 = g_0276bee8;
      local_a8 = lVar1;
      local_a0 = 1;
      if (g_0276bee8 != 0) {
        FUN_00d50b00();
      }
      local_98 = lVar4;
      local_90 = 1;
      FUN_00bf1030(&local_98,&local_a8);
      if (lVar4 != 0) {
        FUN_00d50b20();
      }
      if (lVar1 != 0) {
        FUN_00d50b20();
      }
      lVar1 = g_0276bef0;
      if (g_0276bef0 != 0) {
        FUN_00d50b00();
      }
      lVar4 = g_0276bef8;
      local_88 = lVar1;
      local_80 = 1;
      if (g_0276bef8 != 0) {
        FUN_00d50b00();
      }
      local_78 = lVar4;
      local_70 = 1;
      FUN_00bf1030(&local_78,&local_88);
      if (lVar4 != 0) {
        FUN_00d50b20();
      }
      if (lVar1 != 0) {
        FUN_00d50b20();
      }
      lVar1 = g_0276bf00;
      if (g_0276bf00 != 0) {
        FUN_00d50b00();
      }
      lVar4 = g_0276bf08;
      local_68 = lVar1;
      local_60 = 1;
      if (g_0276bf08 != 0) {
        FUN_00d50b00();
      }
      local_58 = lVar4;
      local_50 = 1;
      FUN_00bf1030(&local_58,&local_68);
      if (lVar4 != 0) {
        FUN_00d50b20();
      }
      if (lVar1 != 0) {
        FUN_00d50b20();
      }
      lVar1 = g_0276bf10;
      if (g_0276bf10 != 0) {
        FUN_00d50b00();
      }
      lVar4 = g_0276bf18;
      local_48 = lVar1;
      local_40 = 1;
      if (g_0276bf18 != 0) {
        FUN_00d50b00();
      }
      local_38 = lVar4;
      local_30 = 1;
      FUN_00bf1030(&local_38,&local_48);
      if (lVar4 != 0) {
        FUN_00d50b20();
      }
      if (lVar1 != 0) {
        FUN_00d50b20();
      }
      g_028a5b19 = '\x01';
      FUN_00e8cb70();
    }
    else {
      g_028a5b19 = '\x01';
      FUN_00e8cb70();
    }
    puVar3 = *(void**)(this_ptr + 0xb0);
    if (puVar3 == g_028a5b10) {
      return;
    }
  }
  else {
    puVar3 = *(void**)(this_ptr + 0xb0);
    if (puVar3 == g_028a5b10) {
      return;
    }
  }
  puVar2 = g_028a5b10;
  if (g_028a5b10 != (void*)0x0) {
    FUN_00d50b00();
  }
  *(void**)(this_ptr + 0xb0) = puVar2;
  if (puVar3 == (void*)0x0) {
    return;
  }
  FUN_00d50b20();
  return;
}

