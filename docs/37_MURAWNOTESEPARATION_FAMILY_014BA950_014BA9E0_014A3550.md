...
Il subset clean-room dei consumer `014a3900 / 014a42b0` e' aperto in `rawnotes/interval_gate.*`: test class-gap `1/2`, controllo del terzo vicino, costo pair-arbitration, merge max/OR dei campi chiusi, predicato peak-gate, predicato candidato primario, helper scalari del ranking span/score, planner one-pass del candidato migliore e planner iterativo vector-backed. Il contenitore originale `GNList`, le mutazioni/refcount della lista originale restano fuori perimetro.
