...
- raw-note peer sintetico/post-match `01484bc0` aperto nel subset corrente: constructor `start=current.start`, strength `1.0f`, flag `0x40`; peer observed/protected `0x10` con constructor `014ba9e0(start, 1.0f, 0x10)` e mark su peer linkato; pipeline class `2` matcher-then-propagation su span; pipeline-plan class `1` matcher-then-postprocess con cursore `uVar51`; predicato cleanup `flags & 0x70`
...
- boundary refinement tardo `014ab140` aperto per il subset scalare: peer linkato `+0x40->+0x18`, fallback su minimo segnale nel gap `>= 0.01`, clamp start/end e guard anchor fuori range; target object e `GNList` restano fuori
- pitch matrix bridge `014b3460 / 014b3ce0` aperto per reset/link nearest-neighbour delle righe `MUPitchMatrixPeak`, coda scalare con floor `peak+0x1c > 0.4f`, pitch bin diretto/medio, conversione `exp2(pitchBin/60)*21.533203125`, output anchor/frequency e failure `-1`; costruzione chain `GNList` e smoothing restano fuori
...
- supporto analyzer gate esteso con default `0.01f`, coverage dei linked peers, mediana `_allEnergyRelevantRegionRatio` su ratio consecutivi con gap limit `2.0`, writer parziale dei soli campi `+0x100/+0x104/+0x108`, e nomi canonici `_allTonality / _allCentDiffPerMillisecond / _allPitchRangePerNote / _allEnergyRelevantRegionRatio / _allAttackPeakQuality / _allAttackPeakToEnergyRegionRatio`
