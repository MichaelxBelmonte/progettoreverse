...
- raw-note paired peak-gate kernel aperto per `014a3550`: state half-smoothing a due lane, peak max e write su `+0x34/+0x38`; il writer completo resta fuori per i campi `+0x2c/+0x30`
- raw-note interval gate subset aperto per `014a3900 / 014a42b0`: costo pair-arbitration, merge max/OR dei campi chiusi e predicato peak-gate; ranking gap completo e mutazioni lista restano fuori
...
Nota raw-note builder-side: `014ba9e0` e' ora chiuso come ABI `xmm0=start`, `xmm1=base strength`, `esi=flag`. `014a2170` genera candidate `1/2` da run positive con massimo locale sopra `g_0239424c` come `+0x20`; `014afb20` genera candidate `8` da contrasto locale sopra `g_02391090`; `01484bc0` genera peer sintetici `0x40` con `+0x20 = 1.0f`. Le soglie/finestra principali sono ora lette dal binario: `g_0239424c = 0.0f`, `g_02391090 = 0.100000001f`, `g_02390108 = 0.05`, `g_0240e400 = 0.0227`, `g_0240f0f0 = 441.0`, `g_02394254 = 0.01f`, smoothing factors `5.0f / 30.0f`. La pipeline classe `8` e' chiusa high-level come delta primo ordine -> moving contrast assoluto -> clamp `4.0f` -> smoother esponenziale breve/lungo `015c1480` mode `0` -> valley segment -> `max - min > 0.1`; il mapping linked-successor item a range protetti e' implementato, mentre resta prudente il nome canonico dell'owner di quegli item. Correzione `+0x34/+0x38`: non sono piu' trattati come lower/upper threshold pair, ma come paired local smoothed peak gates prodotti da `014a3550` con coefficiente `0.5f`, max-merged da `014a3900` e usati in OR da `014a42b0`; il kernel di peak-gate e il subset interval-gate sono ora nel core clean-room, ma non il writer completo ne' il ranking gap completo.

Nota implementazione clean-room `014a2170`: `raw_note_class12_builder.*` espone ora solo il materializzatore da buffer gia' preprocessati per classi `1/2`, con gate durata strict, massimo locale `> 0.0f`, start da peak o riallineamento a item esistente e flag canonici `1/2`. Il preprocessing/baseline `015c2d90` resta escluso dal codice attivo.
