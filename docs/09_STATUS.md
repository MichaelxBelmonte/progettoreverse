...
Nota raw-note builder-side: `014ba9e0` e' ora chiuso come ABI `xmm0=start`, `xmm1=base strength`, `esi=flag`. `014a2170` genera candidate `1/2` da run positive con massimo locale sopra `g_0239424c` come `+0x20`; `014afb20` genera candidate `8` da contrasto locale sopra `g_02391090`; `01484bc0` genera peer sintetici `0x40` con `+0x20 = 1.0f`. Le soglie/finestra principali sono ora lette dal binario: `g_0239424c = 0.0f`, `g_02391090 = 0.100000001f`, `g_02390108 = 0.05`, `g_0240e400 = 0.0227`, `g_0240f0f0 = 441.0`, `g_02394254 = 0.01f`, smoothing factors `5.0f / 30.0f`. La pipeline classe `8` e' chiusa high-level come delta primo ordine -> moving contrast assoluto -> clamp `4.0f` -> smoother esponenziale breve/lungo `015c1480` con `alpha = expf(-2.5f / width)` -> valley segment -> `max - min > 0.1`; per bit identity resta da trascrivere fedelmente l'edge-pass mode `0` di `015c0b60`. Correzione `+0x34/+0x38`: non sono piu' trattati come lower/upper threshold pair, ma come paired local smoothed peak gates prodotti da `014a3550` con coefficiente `0.5f`, max-merged da `014a3900` e usati in OR da `014a42b0`.
