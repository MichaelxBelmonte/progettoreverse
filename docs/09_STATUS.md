...
- raw-note peer sintetico/post-match `01484bc0` aperto nel subset corrente: constructor `start=current.start`, strength `1.0f`, flag `0x40`; propagation class `2` con loop span, claim class `1`, predicato cleanup `flags & 0x70`, piano vector-backed di insert sintetico/cleanup class `1` e sequencer con cursore `uVar51`
