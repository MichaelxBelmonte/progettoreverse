// Function: FUN_00e271f0
// Address: 00e271f0
// Size: 2689 bytes
// Class: GNString
// String references:
//   "GNString"
//   "GNDictionary"


/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void FUN_00e271f0(undefined8 param_1,int param_2)

{
  longlong *plVar1;
  longlong *plVar2;
  longlong lVar3;
  bool bVar4;
  bool bVar5;
  char cVar6;
  int iVar7;
  longlong *plVar8;
  longlong *plVar9;
  longlong *unaff_RSI;
  longlong *unaff_RDI;
  longlong **pplVar10;
  longlong lVar11;
  undefined4 uVar12;
  undefined4 extraout_XMM0_Da;
  undefined4 extraout_XMM0_Da_00;
  undefined4 extraout_XMM0_Da_01;
  longlong local_118;
  char local_110;
  longlong *local_108;
  char local_100;
  longlong *local_f8;
  char local_f0;
  longlong *local_e8;
  char local_e0;
  longlong local_d8;
  char local_d0;
  longlong *local_c8;
  char local_c0;
  longlong *local_a8;
  longlong *local_60;
  char local_58;
  longlong local_50;
  undefined8 local_48;
  undefined4 local_40;
  longlong *local_38;
  
  plVar8 = (longlong *)FUN_00e8fc40();
  FUN_0004b090();
  uVar12 = (**(code **)(*plVar8 + 0x18))();
  if (param_2 != 0) {
    local_c0 = 0;
    (**(code **)(unaff_RDI[2] + 0x10))();
    FUN_00d50b00();
    local_c0 = '\x01';
    local_c8 = unaff_RDI + 2;
    uVar12 = FUN_00d1fc70();
    if ((local_c0 != '\0') && (local_c8 != (longlong *)0x0)) {
      (**(code **)(*local_c8 + 0x10))();
      uVar12 = FUN_00d50b20();
    }
  }
  local_118 = *unaff_RSI;
  local_110 = '\0';
  FUN_00d1b500(uVar12,&local_118);
  local_a8 = local_60;
  if (local_58 == '\0') {
    if (local_60 == (longlong *)0x0) {
      local_a8 = (longlong *)0x0;
    }
    else {
      FUN_00d50b00();
      if ((local_58 != '\0') && (local_60 != (longlong *)0x0)) {
        FUN_00d50b20();
      }
    }
  }
  else {
    local_a8 = local_60;
    local_58 = '\0';
  }
  if ((local_110 != '\0') && (local_118 != 0)) {
    FUN_00d50b20();
  }
  if (local_a8 == (longlong *)0x0) goto LAB_00e27940;
  if ((DAT_026fdd40 == '\0') && (iVar7 = ___cxa_guard_acquire(), iVar7 != 0)) {
    _DAT_026cd0e8 = FUN_00d4fe50();
    DAT_026cd0d0 = "GNDictionary";
    _DAT_026cd0d8 = 0x28;
    _DAT_026cd0e0 = FUN_00022d20;
    _DAT_026cd0f0 = 0;
    uRam00000000026cd0f8 = 0;
    _DAT_026cd100 = 0;
    _DAT_026cd178 = 0;
    uRam00000000026cd180 = 0;
    _DAT_026cd188 = 0;
    DAT_026cd18a = 6;
    _DAT_026cd108 = 0;
    uRam00000000026cd110 = 0;
    _DAT_026cd118 = 0;
    uRam00000000026cd120 = 0;
    _DAT_026cd128 = 0;
    uRam00000000026cd130 = 0;
    _DAT_026cd138 = 0;
    uRam00000000026cd140 = 0;
    _DAT_026cd148 = 0;
    uRam00000000026cd150 = 0;
    _DAT_026cd158 = 0;
    uRam00000000026cd160 = 0;
    _DAT_026cd168 = 0;
    uRam00000000026cd170 = 0;
    DAT_026cd193 = 0;
    _DAT_026cd18b = 0;
    ___cxa_guard_release();
  }
  (**(code **)(*local_a8 + 0x360))();
  cVar6 = FUN_00e85ea0();
  uVar12 = extraout_XMM0_Da;
  if (cVar6 == '\0') {
LAB_00e27353:
    plVar9 = DAT_02802688;
    cVar6 = DAT_02802690;
    if (DAT_02802690 != '\0') goto LAB_00e273b7;
LAB_00e273c6:
    if (plVar9 != (longlong *)0x0) goto LAB_00e273cb;
    plVar9 = (longlong *)FUN_00e8fc40();
    FUN_00022d50();
    uVar12 = (**(code **)(*plVar9 + 0x18))();
    plVar2 = DAT_02785c30;
    local_108 = local_a8;
    local_100 = '\0';
    if (DAT_02785c30 != (longlong *)0x0) {
      uVar12 = FUN_00d50b00();
    }
    local_60 = plVar2;
    local_58 = '\0';
    uVar12 = FUN_00ca0840(uVar12,&local_60);
    if ((local_58 != '\0') && (local_60 != (longlong *)0x0)) {
      uVar12 = FUN_00d50b20();
    }
    if (plVar2 != (longlong *)0x0) {
      uVar12 = FUN_00d50b20();
    }
    if ((local_100 != '\0') && (local_108 != (longlong *)0x0)) {
      uVar12 = FUN_00d50b20();
    }
joined_r0x00e274f3:
    bVar5 = false;
    bVar4 = false;
    plVar2 = DAT_02785c30;
  }
  else {
    if ((DAT_026fd0c0 == '\0') && (iVar7 = ___cxa_guard_acquire(), iVar7 != 0)) {
      _DAT_026d5e58 = FUN_00d4fe50();
      DAT_026d5e40 = "GNString";
      _DAT_026d5e48 = 0x40;
      _DAT_026d5e50 = FUN_0005d920;
      _DAT_026d5e60 = 0;
      uRam00000000026d5e68 = 0;
      _DAT_026d5e70 = 0;
      uRam00000000026d5e78 = 0;
      _DAT_026d5e80 = 0;
      uRam00000000026d5e88 = 0;
      _DAT_026d5e90 = 0;
      uRam00000000026d5e98 = 0;
      _DAT_026d5ea0 = 0;
      uRam00000000026d5ea8 = 0;
      _DAT_026d5eb0 = 0;
      uRam00000000026d5eb8 = 0;
      _DAT_026d5ec0 = 0;
      uRam00000000026d5ec8 = 0;
      _DAT_026d5ed0 = 0;
      uRam00000000026d5ed8 = 0;
      _DAT_026d5ee0 = 0;
      uRam00000000026d5ee8 = 0;
      _DAT_026d5ef0 = 0;
      uRam00000000026d5ef8 = 0;
      _DAT_026d5f00 = 0;
      ___cxa_guard_release();
    }
    cVar6 = FUN_00e8db60();
    uVar12 = extraout_XMM0_Da_00;
    if (cVar6 == '\0') goto LAB_00e27353;
    cVar6 = '\x01';
    plVar9 = local_a8;
LAB_00e273b7:
    if (plVar9 == (longlong *)0x0) goto LAB_00e273c6;
    uVar12 = FUN_00d50b00();
LAB_00e273cb:
    if (cVar6 != '\0') {
      FUN_00d50b00();
      uVar12 = FUN_00d50b20();
      goto joined_r0x00e274f3;
    }
    bVar5 = true;
    bVar4 = true;
    plVar2 = DAT_02785c30;
  }
  DAT_02785c30 = plVar2;
  if (plVar2 != (longlong *)0x0) {
    uVar12 = FUN_00d50b00();
    bVar4 = bVar5;
  }
  local_f0 = '\x01';
  local_f8 = plVar2;
  FUN_000175c0(uVar12,&local_f8);
  plVar2 = local_60;
  if (local_58 == '\0') {
    if (((local_60 != (longlong *)0x0) && (FUN_00d50b00(), local_58 != '\0')) &&
       (local_60 != (longlong *)0x0)) {
      FUN_00d50b20();
    }
  }
  else {
    local_58 = '\0';
  }
  if ((local_f0 != '\0') && (local_f8 != (longlong *)0x0)) {
    FUN_00d50b20();
  }
  if (plVar2 != (longlong *)0x0) {
    local_58 = '\0';
    local_60 = plVar2;
    FUN_00d243f0();
    if ((local_58 != '\0') && (local_60 != (longlong *)0x0)) {
      FUN_00d50b20();
    }
    FUN_00d50b20();
  }
  plVar2 = DAT_027092d0;
  if (DAT_027092d0 != (longlong *)0x0) {
    FUN_00d50b00();
  }
  local_60 = plVar2;
  local_58 = '\0';
  cVar6 = FUN_00c9ff50();
  uVar12 = extraout_XMM0_Da_01;
  if ((local_58 != '\0') && (local_60 != (longlong *)0x0)) {
    uVar12 = FUN_00d50b20();
  }
  if (plVar2 != (longlong *)0x0) {
    uVar12 = FUN_00d50b20();
  }
  plVar2 = DAT_027092d0;
  if (cVar6 != '\0') {
    if (DAT_027092d0 != (longlong *)0x0) {
      uVar12 = FUN_00d50b00();
    }
    local_e8 = plVar2;
    local_e0 = '\x01';
    FUN_000175c0(uVar12,&local_e8);
    plVar2 = local_60;
    if ((DAT_026fdd40 == '\0') && (iVar7 = ___cxa_guard_acquire(), iVar7 != 0)) {
      local_38 = plVar9;
      _DAT_026cd0e8 = FUN_00d4fe50();
      DAT_026cd0d0 = "GNDictionary";
      _DAT_026cd0d8 = 0x28;
      _DAT_026cd0e0 = FUN_00022d20;
      _DAT_026cd0f0 = 0;
      uRam00000000026cd0f8 = 0;
      _DAT_026cd100 = 0;
      _DAT_026cd178 = 0;
      uRam00000000026cd180 = 0;
      _DAT_026cd188 = 0;
      DAT_026cd18a = 6;
      _DAT_026cd108 = 0;
      uRam00000000026cd110 = 0;
      _DAT_026cd118 = 0;
      uRam00000000026cd120 = 0;
      _DAT_026cd128 = 0;
      uRam00000000026cd130 = 0;
      _DAT_026cd138 = 0;
      uRam00000000026cd140 = 0;
      _DAT_026cd148 = 0;
      uRam00000000026cd150 = 0;
      _DAT_026cd158 = 0;
      uRam00000000026cd160 = 0;
      _DAT_026cd168 = 0;
      uRam00000000026cd170 = 0;
      DAT_026cd193 = 0;
      _DAT_026cd18b = 0;
      ___cxa_guard_release();
      plVar9 = local_38;
    }
    pplVar10 = &DAT_02802688;
    if (plVar2 != (longlong *)0x0) {
      (**(code **)(*plVar2 + 0x360))();
      cVar6 = FUN_00e85ea0();
      if (cVar6 != '\0') {
        if ((DAT_026fd0c0 == '\0') && (iVar7 = ___cxa_guard_acquire(), iVar7 != 0)) {
          local_38 = plVar9;
          _DAT_026d5e58 = FUN_00d4fe50();
          DAT_026d5e40 = "GNString";
          _DAT_026d5e48 = 0x40;
          _DAT_026d5e50 = FUN_0005d920;
          _DAT_026d5e60 = 0;
          uRam00000000026d5e68 = 0;
          _DAT_026d5e70 = 0;
          uRam00000000026d5e78 = 0;
          _DAT_026d5e80 = 0;
          uRam00000000026d5e88 = 0;
          _DAT_026d5e90 = 0;
          uRam00000000026d5e98 = 0;
          _DAT_026d5ea0 = 0;
          uRam00000000026d5ea8 = 0;
          _DAT_026d5eb0 = 0;
          uRam00000000026d5eb8 = 0;
          _DAT_026d5ec0 = 0;
          uRam00000000026d5ec8 = 0;
          _DAT_026d5ed0 = 0;
          uRam00000000026d5ed8 = 0;
          _DAT_026d5ee0 = 0;
          uRam00000000026d5ee8 = 0;
          _DAT_026d5ef0 = 0;
          uRam00000000026d5ef8 = 0;
          _DAT_026d5f00 = 0;
          ___cxa_guard_release();
          plVar9 = local_38;
        }
        cVar6 = FUN_00e8db60();
        pplVar10 = &local_60;
        if (cVar6 == '\0') {
          pplVar10 = &DAT_02802688;
        }
      }
    }
    plVar1 = (longlong *)unaff_RDI[10];
    plVar2 = *pplVar10;
    if (plVar1 != plVar2) {
      if (*(char *)(pplVar10 + 1) == '\0') {
        if (plVar2 == (longlong *)0x0) {
          unaff_RDI[10] = 0;
        }
        else {
          FUN_00d50b00();
          plVar1 = (longlong *)unaff_RDI[10];
          unaff_RDI[10] = (longlong)*pplVar10;
        }
      }
      else {
        *(undefined1 *)(pplVar10 + 1) = 0;
        unaff_RDI[10] = (longlong)plVar2;
      }
      if (plVar1 != (longlong *)0x0) {
        FUN_00d50b20();
      }
    }
    if ((local_58 != '\0') && (local_60 != (longlong *)0x0)) {
      FUN_00d50b20();
    }
    if ((local_e0 != '\0') && (local_e8 != (longlong *)0x0)) {
      FUN_00d50b20();
    }
  }
  (**(code **)(*(longlong *)*unaff_RSI + 0x390))();
  plVar2 = local_60;
  if (local_58 == '\0') {
    if (local_60 != (longlong *)0x0) {
      FUN_00d50b00();
    }
  }
  else {
    local_58 = '\0';
  }
  uVar12 = (**(code **)(*unaff_RDI + 0x370))();
  if (plVar2 != (longlong *)0x0) {
    uVar12 = FUN_00d50b20();
  }
  if ((local_58 != '\0') && (local_60 != (longlong *)0x0)) {
    uVar12 = FUN_00d50b20();
  }
  lVar3 = DAT_02785c38;
  if (DAT_02785c38 != 0) {
    uVar12 = FUN_00d50b00();
  }
  local_d8 = lVar3;
  local_d0 = '\x01';
  FUN_000175c0(uVar12,&local_d8);
  plVar2 = local_60;
  if (local_58 == '\0') {
    if (local_60 != (longlong *)0x0) {
      FUN_00d50b00();
    }
  }
  else {
    local_58 = '\0';
  }
  (**(code **)(*unaff_RDI + 0x460))();
  if (plVar2 != (longlong *)0x0) {
    FUN_00d50b20();
  }
  if ((local_58 != '\0') && (local_60 != (longlong *)0x0)) {
    FUN_00d50b20();
  }
  if ((local_d0 != '\0') && (local_d8 != 0)) {
    FUN_00d50b20();
  }
  lVar3 = unaff_RDI[4];
  if (lVar3 != 0) {
    local_58 = '\0';
    local_60 = (longlong *)0x0;
    local_40 = 0;
    local_48 = 0;
    local_50 = lVar3;
    local_38 = plVar9;
    if (0 < *(int *)(lVar3 + 0xc)) {
      lVar11 = 0;
      do {
        local_60 = *(longlong **)(*(longlong *)(lVar3 + 0x10) + lVar11 * 8);
        FUN_00d50b00();
        FUN_00c807c0();
        if (unaff_RDI != (longlong *)0x0) {
          FUN_00d50b20();
        }
        lVar11 = lVar11 + 1;
        local_48 = CONCAT44(local_48._4_4_,(int)lVar11);
      } while ((int)lVar11 < *(int *)(lVar3 + 0xc));
    }
    FUN_00e2c740();
    plVar9 = local_38;
  }
  if (!bVar4 && plVar9 != (longlong *)0x0) {
    FUN_00d50b20();
  }
LAB_00e27940:
  if (local_a8 != (longlong *)0x0) {
    FUN_00d50b20();
  }
  if (plVar8 != (longlong *)0x0) {
    FUN_00d50b20();
  }
  return;
}


